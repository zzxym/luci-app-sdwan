...
sdwancmd = s:taboption("general", ListValue, "sdwancmd", translate("Startup Method"),
...
sdwancmd.default = "sdwancmd"
sdwancmd:value("sdwancmd", translate("Default"))
sdwancmd:value("config", translate("Configuration File"))
sdwancmd:value("web", translate("Web Configuration"))
...
et_config.rows = 18
et_config.wrap = "off"
et_config:depends("sdwancmd", "config")
...
web_config:depends("sdwancmd", "web")
...
network_name:depends("sdwancmd", "sdwancmd")
...
network_secret:depends("sdwancmd", "sdwancmd")
...
ip_dhcp:depends("sdwancmd", "sdwancmd")
...
ipaddr:depends("sdwancmd", "sdwancmd")
...
ip6addr:depends("sdwancmd", "sdwancmd")
...
peeradd:depends("sdwancmd", "sdwancmd")
...
external_node:depends("sdwancmd", "sdwancmd")
]=]
...
proxy_network:depends("sdwancmd", "sdwancmd")
...
rpc_portal:depends("sdwancmd", "sdwancmd")
...
rpc_portal_whitelist:depends("sdwancmd", "sdwancmd")
...
listenermode:depends("sdwancmd", "sdwancmd")
...
ws_port:depends("sdwancmd", "web")

wss_port = s:taboption("general", Value, "wss_port", translate("WSS Port"),
        translate("WSS protocol port number: 10012 means WSS will listen on port 10012.<br>"
                .. "If this is the Web configuration in the config file, please fill in the same listening port for firewall allowance."))
wss_port.datatype = "range(1,65535)"
wss_port.default = "10012"
wss_port:depends("listenermode", "ON")
wss_port:depends("sdwancmd", "web")
...
quic_port:depends("sdwancmd", "web")
...
desvice_name:depends("sdwancmd", "sdwancmd")
desvice_name:depends("sdwancmd", "web")
...
uuid.rows = 1
uuid.wrap = "off"
uuid:depends("sdwancmd", "web")
uuid.cfgvalue = function(self, section)
    return nixio.fs.readfile("/etc/sdwan/sdwan_machine_id") or ""
end
uuid.write = function(self, section, value)
    nixio.fs.writefile("/etc/sdwan/sdwan_machine_id", value:gsub("\r\n", "\n"))
end
...
instance_name:depends("sdwancmd", "sdwancmd")
...
vpn_portal:depends("sdwancmd", "sdwancmd")
...
default_protocol:depends("sdwancmd", "sdwancmd")
...
tunname.placeholder = "sdwan0"
tunname:depends("sdwancmd", "sdwancmd")
tunname:depends("sdwancmd", "web")
...
disable_encryption:depends("sdwancmd", "sdwancmd")
...
encryption_algorithm:depends("sdwancmd", "sdwancmd")
...
multi_thread:depends("sdwancmd", "sdwancmd")
...
multi_thread_count:depends("sdwancmd", "sdwancmd")
...
disable_ipv6:depends("sdwancmd", "sdwancmd")
...
latency_first:depends("sdwancmd", "sdwancmd")
...
comp:depends("sdwancmd", "sdwancmd")
...
exit_node:depends("sdwancmd", "sdwancmd")
...
exit_nodes:depends("sdwancmd", "sdwancmd")
...
smoltcp:depends("sdwancmd", "sdwancmd")
...
no_tun:depends("sdwancmd", "sdwancmd")
...
proxy_forward:depends("sdwancmd", "sdwancmd")
...
manual_routes:depends("sdwancmd", "sdwancmd")
...
relay_network:depends("sdwancmd", "sdwancmd")
...
socks_port:depends("sdwancmd", "sdwancmd")
...
disable_p2p:depends("sdwancmd", "sdwancmd")
...
disable_udp:depends("sdwancmd", "sdwancmd")
...
relay_all:depends("sdwancmd", "sdwancmd")
...
bind_device:depends("sdwancmd", "sdwancmd")
...
kcp_proxy = s:taboption("privacy", Flag, "kcp_proxy", translate("Enable KCP Proxy"),
        translate("Convert TCP traffic to KCP traffic to reduce latency and improve speed.<br>"
                .. "All nodes in the virtual network must be using sdwan version v2.2.0 or higher for this feature. "
                .. "(--enable-kcp-proxy parameter)"))
kcp_proxy:depends("sdwancmd", "sdwancmd")
...
kcp_input:depends("sdwancmd", "sdwancmd")
...
quic_proxy = s:taboption("privacy", Flag, "quic_proxy", translate("Enable QUIC Proxy"),
        translate("Proxy tcp streams with QUIC, improving the latency and throughput on the network with udp packet loss.<br>"
                .. "All nodes in the virtual network must be using sdwan version v2.3.2 or higher for this feature. "
                .. "(--enable-quic-proxy parameter)"))
quic_proxy:depends("sdwancmd", "sdwancmd")
...
quic_input:depends("sdwancmd", "sdwancmd")
...
port_forward:depends("sdwancmd", "sdwancmd")
...
accept_dns:depends("sdwancmd", "sdwancmd")
...
private_mode:depends("sdwancmd", "sdwancmd")
...
foreign_relay_bps_limit:depends("sdwancmd", "sdwancmd")
...
extra_args:depends("sdwancmd", "sdwancmd")

log = s:taboption("general", ListValue, "log", translate("Program Log"),
        translate("Runtime log is located at /tmp/sdwan.log. View it in the log section above.<br>"
...
btn0.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli node >/tmp/sdwan-cli_node 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_node")
    end
end
...
btn0info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_node") or ""
...
btn1.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli peer >/tmp/sdwan-cli_peer 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_peer")
    end
end
...
btn6.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli peer-center >/tmp/sdwan-cli_peer-center 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_peer-center")
    end
end
...
btn6info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_peer-center") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btn7.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli vpn-portal >/tmp/sdwan-cli_vpn-portal 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_vpn-portal")
    end
end
...
btn7info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_vpn-portal") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btn8.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli proxy >/tmp/sdwan-cli_proxy 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_proxy")
    end
end
...
btn8info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_proxy") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btn9.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli acl stats >/tmp/sdwan-cli_acl 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_acl")
    end
end
...
btn9info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_acl") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btn10.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli mapped-listener >/tmp/sdwan-cli_mapped_listener 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_mapped_listener")
    end
end
...
btn10info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_mapped_listener") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btn11.write = function()
    if process_status ~= "" then
        luci.sys.call("$(dirname $(uci -q get sdwan.@sdwan[0].sdwanbin))/sdwan-cli stats >/tmp/sdwan-cli_stats 2>&1")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan-cli_stats")
    end
end
...
btn11info.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan-cli_stats") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
btn5.write = function()
    if process_status ~= "" then
        luci.sys.call("echo $(cat /proc/$(pidof sdwan-core)/cmdline | awk '{print $1}') >/tmp/sdwan_cmd")
    else
        luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/sdwan_cmd")
    end
end
...
btn5cmd.cfgvalue = function(self, section)
    local content = nixio.fs.readfile("/tmp/sdwan_cmd") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
sdwanbin = s:taboption("upload", Value, "sdwanbin", translate("sdwan-core Binary Path"),
...
sdwanbin.placeholder = "/usr/bin/sdwan-core"
sdwanbin.default = "/usr/bin/sdwan-core"
...
um.template = "sdwan/other_dvalue"
    ...
    function(meta, chunk, eof)
        ...
        if eof and fd then
            ...
            if string.sub(meta.file, -4) == ".zip" then
                ...
                local extracted_dir = "/tmp/sdwan-linux-*/"
                os.execute("mv " .. extracted_dir .. "sdwan-cli /tmp/sdwan-cli")
                os.execute("mv " .. extracted_dir .. "sdwan-core /tmp/sdwan-core")
                os.execute("mv " .. extracted_dir .. "sdwan-web-embed /tmp/sdwan-web-embed")
                if nixio.fs.access("/tmp/sdwan-cli") then
                    um.value = um.value .. "\n" .. translate("- Program /tmp/sdwan-cli uploaded successfully, restart the plugin to take effect")
                end
                if nixio.fs.access("/tmp/sdwan-core") then
                    um.value = um.value .. "\n" .. translate("- Program /tmp/sdwan-core uploaded successfully, restart the plugin to take effect")
                end
                if nixio.fs.access("/tmp/sdwan-web-embed") then
                    um.value = um.value .. "\n" .. translate("- Program /tmp/sdwan-web uploaded successfully, restart the plugin to take effect")
                end
            end
            ...
            os.execute("chmod +x /tmp/sdwan-core")
            os.execute("chmod +x /tmp/sdwan-cli")
		    os.execute("chmod +x /tmp/sdwan-web-embed")
        end
    end
)
...
-- sdwan-web
s=m:section(TypedSection, "sdwanweb", translate("Self-hosted Web Server"))
s.addremove=false
s.anonymous=true
...
db_path.default = "/etc/sdwan/et.db"

web_protocol = s:option(ListValue, "web_protocol", translate("Listening Protocol"),
        translate("Configure the server's listening protocol for sdwan-core to connect. (-p parameter)"))
...
web_port = s:option(Value, "web_port", translate("Server Port"),
        translate("Configure the server's listening port for sdwan-core to connect. (-c parameter)"))
...
geoip_db.placeholder = "/etc/sdwan/GeoLite.mmdb"

weblog = s:option(ListValue, "weblog", translate("Program Log"),
        translate("Runtime log located at /tmp/sdwanweb.log, viewable in the log section above.<br>"
                .. "Levels: Error < Warning < Info < Debug < Trace"))
weblog.default = "off"
