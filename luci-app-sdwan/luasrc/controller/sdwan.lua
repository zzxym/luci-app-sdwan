...
function index()
	if not nixio.fs.access("/etc/config/sdwan") then
        ...
        entry({"admin", "services", "sdwan"}, alias("admin", "services", "sdwan", "sdwan"),_("SDWAN"), 46).dependent = true
	entry({"admin", "services", "sdwan", "sdwan"}, cbi("sdwan"),_("SDWAN"), 47).leaf = true
	entry({"admin", "services",  "sdwan",  "sdwan_log"}, form("sdwan_log"),_("core log"), 48).leaf = true
	entry({"admin", "services", "sdwan", "get_log"}, call("get_log")).leaf = true
	entry({"admin", "services", "sdwan", "clear_log"}, call("clear_log")).leaf = true
	entry({"admin", "services",  "sdwan",  "sdwanweb_log"}, form("sdwanweb_log"),_("web log"), 49).leaf = true
	entry({"admin", "services", "sdwan", "get_wlog"}, call("get_wlog")).leaf = true
	entry({"admin", "services", "sdwan", "clear_wlog"}, call("clear_wlog")).leaf = true
	entry({"admin", "services", "sdwan", "status"}, call("act_status")).leaf = true
	entry({"admin", "services", "sdwan", "conninfo"}, call("act_conninfo")).leaf = true
end
...
function act_status()
	...
	if cached_newtag and cached_newtag ~= "" then
		...
		e.etnewtag = safe_exec("curl -L -k -s --connect-timeout 3 --user-agent 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Safari/537.36' https://api.github.com/repos/sdwan/sdwan/releases/latest | grep tag_name | sed 's/[^0-9.]*//g'")
		if e.etnewtag ~= "" then
